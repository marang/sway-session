...
require_fixed .goreleaser.yaml 'herdr (>= 0.8.2)'
require_fixed .goreleaser.yaml 'herdr >= 0.8.2'
reject_regex .goreleaser.yaml '^[[:space:]]+- herdr$'
...
require_fixed PKGBUILD 'libpulse: enable sound-reactive presets through parec'
require_fixed PKGBUILD 'alacritty: default terminal adapter for sway-session'
require_fixed PKGBUILD 'herdr>=0.8.2: enable persistent managed terminal sessions'
require_fixed PKGBUILD 'flatpak: launch registered Flatpak applications'
require_fixed PKGBUILD 'foot: alternative terminal adapter for sway-session'
require_fixed PKGBUILD 'glib2: launch registered desktop applications through gio'
require_fixed PKGBUILD 'apparmor: load the optional Codex boundary policy'
require_fixed PKGBUILD 'python: run the optional Codex boundary verification'
require_fixed .SRCINFO 'depends = sway'
require_fixed .SRCINFO 'makedepends = go>=1.26.5'
require_fixed .SRCINFO 'optdepends = libpulse: enable sound-reactive presets through parec'
require_fixed .SRCINFO 'optdepends = alacritty: default terminal adapter for sway-session'
require_fixed .SRCINFO 'optdepends = foot: alternative terminal adapter for sway-session'
require_fixed .SRCINFO 'optdepends = herdr>=0.8.2: enable persistent managed terminal sessions'
require_fixed .SRCINFO 'optdepends = flatpak: launch registered Flatpak applications'
require_fixed .SRCINFO 'optdepends = glib2: launch registered desktop applications through gio'
require_fixed .SRCINFO 'optdepends = apparmor: load the optional Codex boundary policy'
require_fixed .SRCINFO 'optdepends = python: run the optional Codex boundary verification'
